...
async function newPage(url) {
	...
	let done = null
	try {
		const browserType = browserTypes[process.env.BROWSER]
		const args = []
		...
		await page.setViewport({ width: 1440, height: 900, deviceScaleFactor: process.env.BROWSER === Safari ? 2 : undefined })
